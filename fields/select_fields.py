...
nis = re.compile(r"NIS:?\s*(\d+)")
fecha_emision = re.compile(r"Fecha de Emisién: (\d{2}/\d{2}/\d{4})")
consumo_electricidad = re.compile(r"\s*(\d+)\s*KWh")
...
pagar = re.compile(r"TOTAL?\s*Q\s*(\d{1,4}(?:,\d{3})*(?:\.\d{2})?)")
...
def getImportantFields(text) -> dict[str, str]:
    ...
    nit = nit_match.group(1) if nit_match else "NIS no encontrado"
